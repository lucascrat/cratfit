-- SCRIPT DE INSTALAÇÃO COMPLETA: Schema app_correcrat
-- ATENÇÃO: Isso apaga e recria o schema app_correcrat e todas as suas tabelas.

-- 1. Limpeza Inicial (Fresh Start)
DROP SCHEMA IF EXISTS app_correcrat CASCADE;

-- 2. Criar Schema
CREATE SCHEMA app_correcrat;
...
CREATE TABLE app_correcrat.users (
...
CREATE TABLE app_correcrat.activities (
...
-- Tabela: communities
CREATE TABLE app_correcrat.communities (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    name TEXT NOT NULL,
    description TEXT,
    cover_image TEXT,
...
CREATE TABLE app_correcrat.community_members (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
-- Tabela: friendships
CREATE TABLE app_correcrat.friendships (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID REFERENCES app_correcrat.users(id) ON DELETE CASCADE,
...
-- Tabela: achievements
CREATE TABLE app_correcrat.achievements (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    name TEXT NOT NULL,
    description TEXT,
    icon TEXT,
...
CREATE TABLE app_correcrat.user_achievements (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID REFERENCES app_correcrat.users(id) ON DELETE CASCADE,
...
-- Policies: Users
CREATE POLICY "Public Read Users" ON app_correcrat.users
    FOR SELECT USING (true);

CREATE POLICY "Users Updated Own Profile" ON app_correcrat.users
    FOR UPDATE USING (auth.uid() = id);

CREATE POLICY "Users Insert Own Profile" ON app_correcrat.users
    FOR INSERT WITH CHECK (auth.uid() = id);

-- Policies: Activities
CREATE POLICY "Read Public Activities" ON app_correcrat.activities
    FOR SELECT USING (is_public = true OR auth.uid() = user_id);

CREATE POLICY "Users Manage Own Activities" ON app_correcrat.activities
    FOR ALL USING (auth.uid() = user_id);

-- Policies: Generics (Simplificadas para funcionar rápido)
CREATE POLICY "Public Read Communities" ON app_correcrat.communities FOR SELECT USING (true);
CREATE POLICY "Authenticated Create Communities" ON app_correcrat.communities FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Public Read Achievements" ON app_correcrat.achievements FOR SELECT USING (true);

-- 6. Configurar Trigger de Criação de Usuário (Auth -> app_correcrat.users)
